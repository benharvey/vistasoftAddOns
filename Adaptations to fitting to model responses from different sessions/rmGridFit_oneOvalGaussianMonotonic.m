function model = rmGridFit_oneOvalGaussianMonotonic(model,prediction,data,params,t)
...
%     end
    ...
    % now update
    model.x0      = b(1,:)./b(2,:);
    model.y0      = zeros(size(model.x0));
    model.s       = zeros(size(model.x0));
    model.s_major = zeros(size(model.x0));
    model.s_minor = zeros(size(model.x0));
    model.s_theta = zeros(size(model.x0));
    model.rss     = rss;
    if length(params.stim)>1 && params.seperateRunBetas;
