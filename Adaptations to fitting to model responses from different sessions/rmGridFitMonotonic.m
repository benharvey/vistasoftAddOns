function view = rmGridFitMonotonic(view,params)
...
%     params2=params;
%     l=length(params2.stim);
%     params2.stim=params2.stim(1:l/2);
...
%     end
    ...
    %Make predictions here so we can see how many predictors to expect from
    %data size
    allstimimages = rmDecimate(params.analysis.allstimimages,...
    params.analysis.coarseDecimate);
    if size(data,1)==(size(allstimimages,1)/2)
%         ratios=10.^(-5:0.05:5);
%         ratios=[fliplr(-ratios) ratios];
        ratios=0:0.001:1;
        n = numel(ratios);%params.analysis.x0);
        prediction = zeros(size(data,1),n,'single');
        
        numColumns=max(allstimimages(1:(size(allstimimages)./2),:),[],1)~=0;
        numPredictor=allstimimages(1:(size(allstimimages)./2), numColumns)*params.analysis.X(numColumns,1);
        sizeColumns=max(allstimimages((size(allstimimages)./2+1):end,:),[],1)~=0;
        sizePredictor=allstimimages((size(allstimimages)./2+1):end,sizeColumns)*(params.analysis.X(sizeColumns,1));
    
        for ii=1:length(ratios)
           prediction(:,ii)=(numPredictor*(1-ratios(ii)))+(sizePredictor.*ratios(ii));
        end
        
        params.analysis.x0         = single(ratios);
        params.analysis.y0         = single(zeros(size(ratios)));
        params.analysis.sigmaMajor = single(ones(size(ratios)));
        params.analysis.sigmaMinor = (params.analysis.sigmaMajor);
        params.analysis.theta      = params.analysis.y0;
        params.analysis.X          = single(params.analysis.X);
        params.analysis.Y          = single(zeros(size(params.analysis.X)));
        params.analysis.allstimimages    = single(params.analysis.allstimimages);
        params.analysis.sigmaRatio       = params.analysis.y0;
        params.analysis.sigmaRatioInfVal = params.analysis.y0;
        params.analysis.sigmaRatioMaxVal = params.analysis.y0;
        
        
    elseif size(data,1)==size(allstimimages,1) && sum(params.analysis.Y)>0 && sum(params.analysis.Y-params.analysis.X)>0 
%         tmp=1+((1./unstimyer(params.analysis.Y))-1).*0.6;
%         tmp=repmat(tmp', [size(allstimimages,1), 1]);
%         prediction(:,1)=sum(allstimimages./tmp,2);  %Transient component,compressive. Needs more work.
        
        prediction(:,1)=sum(allstimimages,2);  %Transient component, linear
        prediction(:,2)=allstimimages*params.analysis.X;
        
        params.analysis.x0         = single([1 2]);
        params.analysis.y0         = zeros(size(params.analysis.x0));
        params.analysis.sigmaMajor = params.analysis.x0;
        params.analysis.sigmaMinor = (params.analysis.sigmaMajor);
        params.analysis.theta      = params.analysis.y0;
        params.analysis.X          = single(params.analysis.X);
        params.analysis.Y          = single(zeros(size(params.analysis.X)));
    ...
    elseif size(data,1)==size(allstimimages,1)
        prediction(:,1)=allstimimages*params.analysis.X;
        prediction(:,2)=0-(allstimimages*params.analysis.X);
    ...
    for n=1:numel(params.analysis.pRFmodel)
        switch lower(params.analysis.pRFmodel{n}),
            ...
            case {'oneovalgaussian','one oval gaussian','one oval gaussian without theta'}
                s{n}=rmGridFit_oneOvalGaussianMonotonic(s{n},prediction,data,params,t);
