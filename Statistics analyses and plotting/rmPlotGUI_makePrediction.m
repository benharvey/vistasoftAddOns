function [prediction, RFs, rfParams, varexp blanks] = rmPlotGUI_makePrediction(M, coords, voxel, seperateRunBetas)
...
if notDefined('seperateRunBetas')
    seperateRunBetas=0;
end    
% Get model and info
...
if sum(unique(M.model{1}.x0))==3; 
    if M.model{1}.x0(coords)==1
        pred=M.params.analysis.allstimimages*M.params.analysis.X;
        RFs = M.params.analysis.X;
    elseif M.model{1}.x0(coords)==2
        pred=0-M.params.analysis.allstimimages*M.params.analysis.X;
        RFs = max(M.params.analysis.X)-M.params.analysis.X;
    else
        pred=ones(size(M.params.analysis.allstimimages,1),1).*mean(M.params.analysis.allstimimages);
    end
    %rmPlotGUI_makeRFs(modelName, rfParams, M.params.analysis.X, M.params.analysis.Y);
else
    %% make RFs
...
%         allstimimages1=M.params.analysis.allstimimages;
%         allstimimages1=allstimimages1./repmat(M.params.analysis.Y', [size(allstimimages1,1), 1]);
%         pred1=allstimimages1*RFs(:,1);
%         pred2=M.params.analysis.allstimimages * RFs(:,2);
%         whichBeta=find(model.roi.coordsIndex==coords);
%         betaRatio=model.beta(1,whichBeta,2)./model.beta(1,whichBeta,1);
%         pred=pred1+betaRatio.*pred2;
% 
%         [x,y]=meshgrid(0:0.01:max(M.params.analysis.X), 0:0.01:max(M.params.analysis.Y));
...
%         RFs(:,2)=RFs(:,2)./max2;
%         
%         RFs=RFs(:,1)+betaRatio.*RFs(:,2);
%         RFs=reshape(RFs, size(x));
        
        
        
        
%         allstimimages1=M.params.analysis.allstimimages;
%         allstimimages1(:,1:2)=0;  %Assuming no response to 1 count
        ...
        allstimimages2=M.params.analysis.allstimimages./repmat(downbeatsPerTR, [size(M.params.analysis.allstimimages,1), 1]);
        allstimimages1=allstimimages2;
        allstimimages1(:,1:2)=0;

        pred1=allstimimages1*RFs(:,1);
        pred2=allstimimages2 * RFs(:,2);
    ...
    elseif length(RFs)>6 %%FIX THIS
        if isfield(M.params.analysis, 'logIntensity') && length(M.params.analysis.logIntensity)==1 && length(M.params.analysis.logIntensity)>0
            y=[0.05:0.05:1 2.1 0.1 0.6 0.9 1 0.15 0.65 0.85 1 0.2 0.7 0.8 1 0.25 0.75 1 0.3 0.7 0.8 1 0.35 0.65 0.85 1 0.4 0.6 0.9 1 0.45 0.55 0.95 1 0.5 1 0.55 1 0.6 1 0.65 1 0.7 1 0.75 1 0.8 1 0.85 1 0.9  1 0.95 1 1 2.1];
            freq=5./y;
            if M.params.analysis.logIntensity==1
                scale=1./log(freq');
            else
                scale=1./(freq'.^M.params.analysis.logIntensity);
            end
            scale=scale.*freq';
            RFs=RFs./scale;
        elseif isfield(M.params.analysis, 'logIntensity')
%             y=[0.05:0.05:1 2.1 0.1 0.6 0.9 1 0.15 0.65 0.85 1 0.2 0.7 0.8 1 0.25 0.75 1 0.3 0.7 0.8 1 0.35 0.65 0.85 1 0.4 0.6 0.9 1 0.45 0.55 0.95 1 0.5 1 0.55 1 0.6 1 0.65 1 0.7 1 0.75 1 0.8 1 0.85 1 0.9  1 0.95 1 1 2.1];
%             freq=5./y;
            freq=1./M.params.analysis.Y';
            scale=1./(freq'.^rfParams(7));
            scale=scale.*freq';
            RFs=RFs./scale;
        end
        ...
        if isfield(M.params.analysis, 'logIntensity') && all(M.params.analysis.logIntensity==1)
            freq=5./y;
           scale=1./log(freq);
           scale=scale.*freq;
           RFs=RFs./scale;         
...
switch modelName,
    case {'2D pRF fit (x,y,sigma, positive only)',...
          ...
          '1D compressive balance (x=balance, y=compression, positive only)',...
          '1D pRF fit with compressive monotonic Y(x,ycomp,sigma, positive only)',...
          '1D pRF fit plus compressive monotonic Y(x,ycomp,sigma, positive only)'};
        if recompFit==0,
            beta = rmCoordsGet(M.viewType, model, 'b', coords);
            beta = beta([1 dcid+1])';
            
        elseif seperateRunBetas==1
            %betaCoords=
            %beta=squeeze(model.beta(1,coords,:));
            framesPerScan=size(trends,1)/length(M.params.stim);
            predictors=zeros(size(pred,1),length(M.params.stim));
            for stimRun=1:length(M.params.stim)
                predictors(((stimRun-1)*framesPerScan+1):(framesPerScan*stimRun), stimRun)=pred(((stimRun-1)*framesPerScan+1):(framesPerScan*stimRun));
            end
            pred=predictors;
            %beta=beta([1:5 dcid+5])';
            try
            ...
            %if mean(beta(1:5))>0
            for bi=1:size(pred,2)
                beta(bi) = max(beta(bi),0);
            end
%             else
%                 beta(1) = min(beta(1),0);
%                 beta(2) = min(beta(2),0);
...
try
prediction = [single(pred) single(trends(:,dcid))] * beta;
catch
   voxel 
end
