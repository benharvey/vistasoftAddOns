function data = rmPlotNumberVsSigmaLin(v, varExpThresh, doSave)
...
thresh.ecc = [0 max([rmParams.stim(:).stimSize])] + [0.05 -0.05]; 
...
% x2fit=[1 5.5]; %fitting eccentricity range for lines
...
% end
% if exist('bootstrp', 'file')
%     B2 = bootstrp(1000,@(x) cmfFit(x,roi.ecc(iib),roi.pi(iib),roi.varexp(iib)),[1:numel(roi.ecc(iib))]);
%     B2 = B2';
%     roi.p2=B2;
%     pct1 = 100*0.05/2;
%     pct2 = 100-pct1;
...
%     keep2 = B2(2,:)>b2_lower(2) &  B2(2,:)<b2_upper(2);
%     keep = keep1 & keep2;
%     x2fit = linspace(min(x2fit),max(x2fit),100)';
%     y2fit=1./(x2fit.*y2fit(1)+y2fit(2));
%     b2_xfit = linspace(min(x2fit),max(x2fit),100)';
%     fits = 1./([b2_xfit ones(100,1) ]*B2(:,keep));
%     b2_upper = max(fits,[],2);
%     b2_lower = min(fits,[],2);
% end
% roi.b2_upper=b2_upper;
% roi.b2_lower=b2_lower;
...
%         b3_lower = min(fits,[],2);
%     end
%     roi.b3_upper=b3_upper;
%     roi.b3_lower=b3_lower;
% end
...
for b=thresh.ecc(1):binsize:thresh.ecc(2),
    ...
    if any(bii),
        ...
        ii2 = find(abs(data.x-b)<0.01);
        
        %Bootstrap to find mean pRF sigma for each bin
        s=bootstrp(1000, @(x) weightedMean(x, roi.sigma(bii), roi.varexp(bii)),[1:sum(bii)]);
        data.y(ii2) = median(roi.sigma(bii));%  prctile(s,50);
        
        s = wstat(roi.sigma(bii),roi.varexp(bii));
        data.ysterr(:,ii2) = s.sterr;
        data.nbii(ii2)=sum(bii);
...
%Bootstrap fit line to bin points
if exist('bootstrp', 'file')
    [B] = bootstrp(1000,@(x) localfit(x,data.x(data.x>=xfit(1) & data.x<=xfit(2) & ~isnan(data.y) & data.ysterr(1,:)'>0.01 & data.nbii>3),data.y(data.x>=xfit(1) & data.x<=xfit(2) & ~isnan(data.y) & data.ysterr(1,:)'>0.01 & data.nbii>3),1./data.ysterr(1,(data.x>=xfit(1) & data.x<=xfit(2) & ~isnan(data.y) & data.ysterr(1,:)'>0.01 & data.nbii>3))' ),[1:sum(data.x>=xfit(1) & data.x<=xfit(2) & ~isnan(data.y) & data.ysterr(1,:)'>0.01 & data.nbii>3)]');
    B = B';
    pct1 = 100*0.05/2;
    pct2 = 100-pct1;
...
%Bootstrap fit line to individual voxels (independence issues)
% if exist('bootstrp', 'file') 
...
function [B,e]=prfFit(ii,x,y,ve)
x = x(ii); y = y(ii); ve = ve(ii);
[B,e] = fminsearch(@(z) myprffit(z,x,y,ve),[0.05;0.2]);
return

function e=myprffit(z,x,y,ve)
