function vw = rmRunOccupancyFreq2d(vw,dataTypes, roi,wSearch,models, hrfParams, matFileName, maxCores, separateBetas)
...
elseif wSearch==13 %This model makes no conceptual sense, should not be run
    ...
    wSearch=1;
elseif wSearch==14
    matFileName = sprintf('retModel-%s-Lin-1dGaussianXlinearY-OccupancyFreq-%s',datestr(now,'yyyymmdd-HHMMSS'), num2str(highestValue,2));   % -Lin-1dGaussianXlinearY-OccupancyFreq-%s',datestr(now,'yyyymmdd-HHMMSS'), num2str(highestValue,2));   OR %s-Lin-Flat-FreqOnly-%s
