function [dataAll] = PlotRoiDistanceAllRoiDuration(data, veThresh, showPlots, binSteps, meanThresh)
...
%         
         %n=1; 
         data(n).veIndices=data(n).ves>=veThresh & data(n).x0s>0.05 & data(n).x0s<1 & data(1).meanSignal>=meanThresh(whichRoi);
...
%         %data(n).sigmafit=linreg(data(n).ratio(data(n).veIndices), data(n).sigmas(data(n).veIndices), data(n).ves(data(n).veIndices));
    ...
    for n=1:length(data)
        ...
        try
        [data(n).ypoints, data(n).logLineFit, data(n).bupper, data(n).blower, data(n).B]=bootstrapLineFitter(data(n).x{4},data(n).y{4},1./data(n).ysterr{4}(1,:), xpoints);
        catch
           n 
        end
...
if showPlots(4)==1
    for n=1:size(dataAll,1)
        ...
        dataAll(n,1).sigmas=dataAll(n,1).sigmas.*(2*sqrt(2*log(2)));
