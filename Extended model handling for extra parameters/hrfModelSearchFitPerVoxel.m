function hrfParams = hrfModelSearchFitPerVoxel(data,prediction,hrfParams,params)
...
gridParams = single(startParams*ones(1,[size(data, 2)]));
...
eDefault=hrfFit(single(startParams),sdata,spred,srawrss,stime,estimate_dc, params.seperateRunBetas);
                    ...
                    if i3>i1+1
                        for i4=linspace(bndParams(4,1),bndParams(4,2),gridsample)
                            ...
                            if i4>=i2
                                ...
                                e=hrfFit(h,sdata,spred,srawrss,stime,estimate_dc, params.seperateRunBetas);
                                %if any(e<eDefault)
                                    gridParams(:,e<eDefault)=h'*ones(1, sum(e<eDefault));
                                    eDefault(e<eDefault) = e(e<eDefault);
                                %end
                            end
                        end
                    end
...
% tic;
% hrfParams = ...
%     fmincon(@(x) hrfFit(x,data,prediction,rawrss,FIR_time,estimate_dc, params.seperateRunBetas),...
...
%-----------------------------------------
function e=hrfFit(params,data,stim,rawrss,t,estimate_dc, seperateRunBetas)
% accuracy of penalizing factors
acc = 0.005;

% assign maximal rss
e = 1e7*ones([1 size(data,2)]);
