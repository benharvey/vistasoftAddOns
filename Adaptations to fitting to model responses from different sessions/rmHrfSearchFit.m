...
for n=1:numel(tmp_param),
    ...
    %Seems like this should come after HRF convolution, on the prediction rather than the images
    %So moved to hrfModelSearchFit at line 161
%     if isfield(tmp_param(n), 'prescanDurationPerFrame')
%         tmp_param(n).images = tmp_param(n).images(tmp_param(n).prescanDurationPerFrame+1:end,:);
%     else
%         tmp_param(n).images = tmp_param(n).images(tmp_param(n).prescanDuration+1:end,:);
%     end
    
    % and time averaging
    %Again, this should come after HRF convolution, so is moved to
    %hrfModelSearchFit around line 170.
    %For compatability, still needs rotating
%    tmp_param(n).images = rmAverageTime(tmp_param(n).images, ...
%                                  tmp_param(n).nUniqueRep)';
    tmp_param(n).images = tmp_param(n).images';
end;
...
desc = lower(rmGet(model{1},'desc'));
switch desc,
    case {'2d prf fit (x,y,sigma, positive only)', '1d prf fit (x,sigma, positive only)'}
    ...
    case {'double 2d prf fit (x,y,sigma,sigma2, center=positive)',...
         ...
         if exist('tmpii','var')
            beta=model{1}.beta;
            model{1}.beta=zeros([size(model{1}.x0, 1) size(model{1}.x0, 2) size(model{1}.beta, 3)]); 
            model{1}.beta(:,tmpii,:)=beta;
        end
...
%         if view.selectedROI ~= 0,
%             beta=model{1}.beta;
%             model{1}.beta=zeros([size(model{1}.beta, 1) size(model{1}.x0, 2), size(model{1}.beta)]);
%             model{1}.beta(:,tmpii,:)=beta;
%         end
%         tmp.prediction = rmHrfSearchFit_twoGaussian(model, params, loopSlices, wProcess,varexp,allstimimages);
%         if view.selectedROI ~= 0,
...
%         end
    ...
    case {'oval 2d prf fit (x,y,sigma_major,sigma_minor,theta)'}
        tmp.prediction = rmHrfSearchFit_oneOvalGaussian(model, params, loopSlices, wProcess,varexp,allstimimages);
