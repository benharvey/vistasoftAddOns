function vw = rmSelect(vw, loadModel, rmFile, titleIn, nFiles)
...
% choose filename:
if ~exist('titleIn','var') || isempty(titleIn)
    title='Choose retinotopic model file name';
else
    title=['Looking for' titleIn];
end
if ~exist('rmFile','var') || isempty(rmFile)
    if exist('nFiles','var') && nFiles>1
