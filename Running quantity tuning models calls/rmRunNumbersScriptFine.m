function [vw] = rmRunNumbersScriptFine(vw,dataTypes, roi,wSearch,models, hrfParams, matFileName, maxCores,separateBetas)
...
if ~exist('matFileName','var') || isempty(matFileName)
    if strcmp(models{1},'1g')
        matFileName = sprintf('retModel-%s-1DGaussian-FullBlanks-DT0.5-Fine',models{1}, datestr(now,'yyyymmdd-HHMMSS')); 
    elseif strcmp(models{1},'dog')
        matFileName = sprintf('retModel-%s-1DDoGaussian-FullBlanks-DT0.5-Fine',models{1}, datestr(now,'yyyymmdd-HHMMSS'));
    end
end
         ...
         switch lower(models{n})
             case '1g'
                ...
                tmpv=rmMain([1 dataTypes(dt)],roi,wSearch,...
                    ...
                    'minrf',.2,'maxrf',14,'numbersigmas',277,... % sample prfs at <0.1 (whatever the unit is)
                    'spacesigmas','linear',...
                    'hrffitthreshve', 0.2,...
                    'hrf', hrfParams,...
