...
switch modelName,
    ...
    case {'2D pRF fit (x,y,sigma_major,sigma_minor)' ...
        ...
        RFs = rfGaussian2d(X, Y, rfParams(3), rfParams(5), rfParams(6), rfParams(1), rfParams(2));
    case {'1D compressive balance (x=balance, y=compression, positive only)'}
        Xval=max(X)-X;
        rfLeft=(Xval.^rfParams(2));
        rfRight=((max(Xval)-Xval).^rfParams(2));
        RFs=rfLeft.*(1-rfParams(1))+rfRight.*rfParams(1);
        
    case {'1D pRF fit with compressive monotonic Y(x,ycomp,sigma, positive only)'}
        RFs = rfGaussianXcompressiveY(X, Y, rfParams(3), rfParams(5), rfParams(6), rfParams(1), rfParams(2));
    case {'1D pRF fit plus compressive monotonic Y(x,ycomp,sigma, positive only)'}
        RFs(:,1) = rfGaussian2d(X, zeros(size(Y)), rfParams(3), rfParams(5), rfParams(6), rfParams(1), zeros(size(rfParams(2))));
        RFs(:,2) = (Y.^rfParams(2))./Y;
