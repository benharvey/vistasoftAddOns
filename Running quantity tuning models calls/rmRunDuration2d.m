function vw = rmRunDuration2d(vw,dataTypes, roi,wSearch,models, hrfParams, matFileName, maxCores, separateBetas, logIntensity)
...
elseif ischar(logIntensity) && strcmp(logIntensity,'free')
    matFileName = sprintf('retModel-%s-Lin-2dCircGaussian-DurationPeriod-DT0.5-maxValue-%s-expIntensity-free',datestr(now,'yyyymmdd-HHMMSS'), num2str(highestValue,2));
    logIntensity=[0.00001 0.2 0.4 0.6 0.8 1];
else
    matFileName = sprintf('retModel-%s-Lin-2dCircGaussian-DurationPeriod-DT0.5-maxValue-%s-expIntensity-%s',datestr(now,'yyyymmdd-HHMMSS'), num2str(highestValue,2), num2str(logIntensity,1));
end
