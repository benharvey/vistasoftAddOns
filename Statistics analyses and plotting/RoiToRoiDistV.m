function view = RoiToRoiDistV(targetROI, sourceROI, view)
...
if sourceROI>targetROI 
    minusflag=0;
else
    minusflag=1;
end
if isnumeric(targetROI) 
    targetROI = view.ROIs(targetROI); 
end
if isnumeric(sourceROI)
    sourceROIno=sourceROI;
    sourceROI = view.ROIs(sourceROI); 
end
...
view.ROIs(sourceROIno).disttoROI=min(allDist, [], 2)';

% % Below code saves ROI to ROI distance in the view.co field instead of
% % the source ROI structure. This allows distances from several ROIs in
% % the same view to be saved in the same field, rather than saving each in
% % its own ROI. However, as not all voxels in the view fall inside an ROI,
% % this view.co field will be sparsely filled, with the rest as NaNs.

% if ~isnan(view.co{1})%==zeros(1, length(view.co{1})
%     view.co{1}=NaN(1,length(view.co{1}));
% end
% [tmp, indices] = intersectCols(view.coords,sourceROI.coords);
% 
% 
% % Find  shortest distance from each voxel in source ROI to target ROI 
% if minusflag==0
%     view.co{1}(1,indices) = min(allDist, [], 2)';
% else
%     view.co{1}(1,indices) =0-(min(allDist, [], 2)');
% end
