function vw = rmRunCueScriptGeneral(vw,dataTypes, roi,wSearch,models, hrfParams, matFileName, maxCores, separateBetas, highestValue, blankValue, lowestValue)
...
if ~exist('matFileName','var') || isempty(matFileName)
    matFileName = sprintf('retModel-%s-1DGaussian-log-FullBlanks-DT0.5',datestr(now,'yyyymmdd-HHMMSS')); 
end
...
if ~exist('lowestValue','var') || isempty(lowestValue)
    lowestValue=0;
end    
...
%highestValue=1.4;
sampleRate=(highestValue-lowestValue)*(0.01);
    ...
    for n=1:numel(models)
        switch lower(models{n})
            case '1g'
                ...
                %matFileName = sprintf('retModel-%s-1DGaussian-FullBlanks-DT0.5',datestr(now,'yyyymmdd-HHMMSS')); 

                tmpv=rmMain([1 dataTypes(dt)],roi,wSearch,...
                    'prf model',{'1dgaussian'},...
                    'coarsetofine',false,...
                    'coarseDecimate',0,... % alldata no blurring
                    'minFieldSize',lowestValue,...
