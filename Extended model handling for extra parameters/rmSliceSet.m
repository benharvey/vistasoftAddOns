...
for n=1:numel(model),
    ...
    switch length(size(val)) 
        ...
        otherwise            
            ...
            val = zeros([size(val,1) size(val,2) size(tmp{n}.b,1)]);

            for fn = 1:size(val,3),
                try
                    val(slice,:,fn) = double(tmp{n}.b(fn,:));
                catch
                    if size(tmp{n}.b,2)>size(val,2) 
                        try
                        val(slice,:,fn) = double(tmp{n}.b(fn,model{1}.roi.coordsIndex));
                        catch
                            %do nothing, beta can be empty
                        end
                    end
                end
            end;
