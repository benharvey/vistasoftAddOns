...
% degrees).
params.analysis.numberStimulusGridPoints = 180;
...
if ~isfield(params, 'matFileName') || isempty(params.matFileName),
...
if ~isfield(params.analysis,'coarseToFine')
...
if ~isfield(params.analysis,'coarseDecimate')
...
if ~isfield(params.analysis,'numberSigmas')
    if strcmp(params.analysis.pRFmodel{1}, 'one oval gaussian') || ...
            strcmp(params.analysis.pRFmodel{1}, 'oneovalgaussian') || ...
...
if ~isfield(params.analysis,'minRF')
...
if ~isfield(params.analysis,'spaceSigmas')
...
if ~isfield(params.analysis,'relativeGridStep')
...
%For timing tuning models, limit to possible timing preferences, where
%duration <= period
% Keep those that fit our rules:
dist = sqrt(x.^2+y.^2);

%For lin norm log models
stimy=log([0.05:0.05:1 2.1])+3;
stimy=stimy-min(stimy);
stimy=(stimy./stimy(end-1)).*0.95;
stimy=stimy+0.05;

if  isfield(params.analysis, 'X') && length(params.analysis.X)==47 && all(params.analysis.X==[0.0238 0.05 0.0526 0.0556 0.0588 0.0625 0.0667 0.0714 0.0769 0.0833 0.0909 0.1 0.1111 0.1250 0.1429 0.15 0.1667 0.1765 0.2 0.2308 0.25 0.2857 0.3 0.3333 0.35 0.3750 0.4 0.4118 0.4286 0.4444 0.45 0.4737 0.5 0.5385 0.55 0.6 0.65 0.6667 0.7 0.75 0.8 0.8182 0.85 0.9 0.95 0.9524 1]) && all(params.analysis.Y==[0.05:0.05:1 2.1])
    disp('Running model with square stimulus space (x<1.1, y<1.1)')
    keep = find(dist<=params.analysis.maxXY & x<=1.1 & y<=1.1 & ...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);  
elseif isfield(params.analysis, 'X') && length(params.analysis.X)==47 && all(params.analysis.X==log([0.0238 0.05 0.0526 0.0556 0.0588 0.0625 0.0667 0.0714 0.0769 0.0833 0.0909 0.1 0.1111 0.1250 0.1429 0.15 0.1667 0.1765 0.2 0.2308 0.25 0.2857 0.3 0.3333 0.35 0.3750 0.4 0.4118 0.4286 0.4444 0.45 0.4737 0.5 0.5385 0.55 0.6 0.65 0.6667 0.7 0.75 0.8 0.8182 0.85 0.9 0.95 0.9524 1])+4) && all(params.analysis.Y==log([0.05:0.05:1 2.1])+4)
    disp('Running model with square stimulus space (x<1.1, y<1.1) (log)')
    keep = find(dist<=params.analysis.maxXY & x<=(log(1.1)+4) & y<=(log(1.1)+4) & ...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);
elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==47 && all(params.analysis.X==[0.0238 0.05 0.0526 0.0556 0.0588 0.0625 0.0667 0.0714 0.0769 0.0833 0.0909 0.1 0.1111 0.1250 0.1429 0.15 0.1667 0.1765 0.2 0.2308 0.25 0.2857 0.3 0.3333 0.35 0.3750 0.4 0.4118 0.4286 0.4444 0.45 0.4737 0.5 0.5385 0.55 0.6 0.65 0.6667 0.7 0.75 0.8 0.8182 0.85 0.9 0.95 0.9524 1]) && all(params.analysis.Y==log([0.05:0.05:1 2.1])+3)
    disp('Running model with square stimulus space (x<1.1, y<1.1) (linlog)')
    keep = find(dist<=params.analysis.maxXY & x<=1.1 & y<=3.1 &...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);  
elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==47 && all(params.analysis.X==[0.0238 0.05 0.0526 0.0556 0.0588 0.0625 0.0667 0.0714 0.0769 0.0833 0.0909 0.1 0.1111 0.1250 0.1429 0.15 0.1667 0.1765 0.2 0.2308 0.25 0.2857 0.3 0.3333 0.35 0.3750 0.4 0.4118 0.4286 0.4444 0.45 0.4737 0.5 0.5385 0.55 0.6 0.65 0.6667 0.7 0.75 0.8 0.8182 0.85 0.9 0.95 0.9524 1]) && all(params.analysis.Y==stimy)
    disp('Running model with square stimulus space (x<1.1, y<1.1) (linnormlog)')
    keep = find(dist<=params.analysis.maxXY & x<=1.1 & y<=1.1 &...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);  
elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==47 && all(params.analysis.X==log([0.0238 0.05 0.0526 0.0556 0.0588 0.0625 0.0667 0.0714 0.0769 0.0833 0.0909 0.1 0.1111 0.1250 0.1429 0.15 0.1667 0.1765 0.2 0.2308 0.25 0.2857 0.3 0.3333 0.35 0.3750 0.4 0.4118 0.4286 0.4444 0.45 0.4737 0.5 0.5385 0.55 0.6 0.65 0.6667 0.7 0.75 0.8 0.8182 0.85 0.9 0.95 0.9524 1])+4) && all(params.analysis.Y==log(1./[0.05:0.05:1 2.1])+1)
    disp('Running model with square stimulus space (x<4.1, y<4.1) (log)')
    keep = find(dist<=params.analysis.maxXY & x<=4.1 & y<=4.1 & x>=0.9 & y>=0.9 &...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);  
elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==47 && all(params.analysis.X==[0.0238 0.05 0.0526 0.0556 0.0588 0.0625 0.0667 0.0714 0.0769 0.0833 0.0909 0.1 0.1111 0.1250 0.1429 0.15 0.1667 0.1765 0.2 0.2308 0.25 0.2857 0.3 0.3333 0.35 0.3750 0.4 0.4118 0.4286 0.4444 0.45 0.4737 0.5 0.5385 0.55 0.6 0.65 0.6667 0.7 0.75 0.8 0.8182 0.85 0.9 0.95 0.9524 1]) && all(params.analysis.Y==1./[0.05:0.05:1 2.1])
    if strcmp(params.analysis.pRFmodel, 'compressive x plus compressive y') || strcmp(params.analysis.pRFmodel, 'linear x plus compressive y') || strcmp(params.analysis.pRFmodel, 'linear x plus linear y')
        %params2=params;
        params.analysis.relativeGridStep=0.01;
        params.analysis.maxXY=sqrt(2);
        [x, y, z]  = sigma2searchgrid(1, params);
        dist = sqrt(x.^2+y.^2);
        keep = find(dist<=params.analysis.maxXY & x>0 & y>0 & x<=1.1 & y<=1.1 & ...
            (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);
    else
        if strcmp(params.analysis.pRFmodel, 'one gaussian x linear y')
            params.analysis.maximumGridSampling=params.analysis.maximumGridSampling*10;
            maxXYtmp=params.analysis.maxXY;
            params.analysis.maxXY=1.1;
            Xtmp=params.analysis.X;
            Ytmp=params.analysis.Y;
            params.analysis.Y=[0.01 1.101];
            params.analysis.X=[0 1.1];
            [x, y, z]  = sigma2searchgrid(initSigmas, params);
            dist = sqrt(x.^2+y.^2);            
            params.analysis.maxXY=maxXYtmp;
            params.analysis.X=Xtmp;
            params.analysis.Y=Ytmp;
        end      
        keep = find(dist<=params.analysis.maxXY & x<=1.1 & y<=1.1 & ...
            (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);
    end
...
elseif strcmp(params.analysis.pRFmodel, 'one gaussian x linear y') && length(params.analysis.X)==210 && all(params.analysis.Y==1)
    params.analysis.maximumGridSampling=params.analysis.maximumGridSampling*10;
    maxXYtmp=params.analysis.maxXY;
    tmp=linspace(0, max(params.analysis.X), params.analysis.maximumGridSampling);
    [x,z]=meshgrid(tmp, initSigmas);
    x=x(:);
    z=z(:);
    y=ones(size(x));
    keep=1:length(x);
    
elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==21 && all(params.analysis.X==[0.05:0.05:1 2]) && all(params.analysis.Y==[0.05:0.05:1 2.1])
    disp('Running model with square stimulus space (x<1.1 y<1.1)')%'Running model with triangle stimulus space (x<1.1*y)')
    keep = find(dist<=params.analysis.maxXY & x<=1.1 & y<=1.1 & x<=1.1.*y &...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);
elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==21 && all(params.analysis.X==log([0.05:0.05:1 2])+3) && all(params.analysis.Y==log([0.05:0.05:1 2.1])+3)
    disp('Running model with triangle stimulus space (x<1.1*y) (log)')
    keep = find(dist<=params.analysis.maxXY & x<=1.1.*y & x<=(log(1.1)+3) & y<=(log(1.1)+3) & ...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);
elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==21 && all(params.analysis.X==[0.05:0.05:1 2]) && all(params.analysis.Y==log([0.05:0.05:1 2.1])+3)
    disp('Running model with triangle stimulus space (x<1.1*y) (linlog)')
    keep = find(dist<=params.analysis.maxXY & x<=1.1.*exp(y-3) & x<=1.1 & y<=1.1 & ...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);
elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==21 && all(params.analysis.X==[0.05:0.05:1 2]) && all(params.analysis.Y==stimy)
    disp('Running model with triangle stimulus space (x<1.1*y) (linnormlog)')
    keep = find(dist<=params.analysis.maxXY & x<=1.1.*unstimyer(y) & x<=1.1 & unstimyer(y)<=1.1 & ...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);

elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==21 && (all(params.analysis.X==[0.05:0.05:1 2])) && all(params.analysis.Y==1./[0.05:0.05:1 2.1])
    if strcmp(params.analysis.pRFmodel, 'compressive x plus compressive y') || strcmp(params.analysis.pRFmodel, 'linear x plus compressive y') || strcmp(params.analysis.pRFmodel, 'linear x plus linear y')
        %params2=params;
        params.analysis.relativeGridStep=0.01;
        params.analysis.maxXY=sqrt(2);
        [x, y, z]  = sigma2searchgrid(1, params);
        %For compressive functions, use round numbers and limit to 0(actually very small but not zero) and 1
        x(x==0)=0.0000001;
        [x, y]=meshgrid(unique(x), unique(x)');
        x=x(:);
        y=y(:);
        dist = sqrt(x.^2+y.^2);
        keep = find(dist<=params.analysis.maxXY & x>0 & y>0 & x<=1 & y<=1);
    else
        if strcmp(params.analysis.pRFmodel, 'one gaussian x linear y')
            params.analysis.maximumGridSampling=params.analysis.maximumGridSampling*10;
            maxXYtmp=params.analysis.maxXY;
...
elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==21 && all(params.analysis.X==[0.05:0.05:1 2]) && all(params.analysis.Y==[0:0.05:0.95 2.05]) 
    disp('Running model with inverted triangle stimulus space (x+y<1.1)')
    keep = find(dist<=params.analysis.maxXY & (x+y)<=1.1 & ...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);
elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==21 && all(params.analysis.X==log([0.05:0.05:1 2])+3) && all(params.analysis.Y==log([0.001 0.05:0.05:0.95 2.05])+3)
    disp('Running model with inverted triangle stimulus space (x+y<1.1) (log)')
    keep = find(dist<=params.analysis.maxXY & (exp(x-3)+exp(y-3))<=1.1 & ...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);

elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==21 && all(params.analysis.X==1./[0.05:0.05:1 2]) && all(params.analysis.Y==1./[0.05:0.05:1 2.1])
    disp('Running model with inverted triangle stimulus space (x>0.9*y)')
    keep = find(dist<=params.analysis.maxXY & x>=.9.*y & x<=24 & ...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);
elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==21 && all(params.analysis.X==log(1./[0.05:0.05:1 2])+1) && all(params.analysis.Y==log(1./[0.05:0.05:1 2.1])+1)
    disp('Running model with inverted triangle stimulus space (x>0.9*y) (log)')
    keep = find(dist<=params.analysis.maxXY & x>=.9.*y & x<=(log(24)+1) & ...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);
elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==21 && all(params.analysis.X==log([0.05:0.05:1 2])+3) && all(params.analysis.Y==log(1./[0.05:0.05:1 2.1])+1)
    disp('Running model with inverted triangle stimulus space (x+y<4.1) (log)')
    keep = find(dist<=params.analysis.maxXY & x+y<=4.1 & x<=(log(1.2)+3) & ...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);
elseif  isfield(params.analysis, 'X') && length(params.analysis.X)==21 && all(params.analysis.X==[0.05:0.05:1 2]) && all(params.analysis.Y==log(1./[0.05:0.05:1 2.1])+1)
    disp('Running model with inverted triangle-ish stimulus space (x+y<4.1) (log)')
    keep = find(dist<=params.analysis.maxXY & log(x)+y<=1.1 & x<=1.1 & y>=0.9 &...
    (dist-params.analysis.outerlimit.*z)<=params.analysis.fieldSize);
elseif strcmp(params.analysis.pRFmodel{1}, 'one gaussian x plus compressive y') || strcmp(params.analysis.pRFmodel{1}, 'one gaussian x compressive y')
    y=y./max(y);
    keep=1:length(x);
else
...
%To allow compressive exponents of stimulus intensity with event frequency
if isfield(params.analysis, 'stimexps') && length(params.analysis.stimexps)>1
    ncandidates=length(params.analysis.x0);
    params.analysis.x0 = repmat(params.analysis.x0, 1, length(params.analysis.stimexps));
    params.analysis.y0 = repmat(params.analysis.y0, 1, length(params.analysis.stimexps));
    params.analysis.sigmaMajor = repmat(params.analysis.sigmaMajor, 1, length(params.analysis.stimexps));
    params.analysis.sigmaMinor = repmat(params.analysis.sigmaMinor, 1, length(params.analysis.stimexps));
    params.analysis.theta = repmat(params.analysis.theta, 1, length(params.analysis.stimexps));
    params.analysis.exponent=repmat(params.analysis.stimexps, ncandidates, 1); 
    
    params.analysis.x0=params.analysis.x0(:);
    params.analysis.y0=params.analysis.y0(:);
    params.analysis.sigmaMajor=params.analysis.sigmaMajor(:);
    params.analysis.sigmaMinor=params.analysis.sigmaMinor(:);
    params.analysis.theta=params.analysis.theta(:);
...
% Where the positions of the stimululus image grid are explicitly passed in.
if isfield(params.analysis, 'X') && isfield(params.analysis,'Y')
    params.analysis.Xin=params.analysis.X;
    params.analysis.Yin=params.analysis.Y;
    [params.analysis.X, params.analysis.Y] = meshgrid(params.analysis.X,params.analysis.Y);
    if isfield(params.analysis, 'Freq')
        params.analysis.Freqin=params.analysis.Freq;
        [~, params.analysis.Freq]=meshgrid(params.analysis.Xin,params.analysis.Freq);
    end
else
    %Where the stimulus image grid is generated.
...
% Allow a different beta amplitude to be fit for each stimulus run. Usful
% if responses to different stimuli are recorded on different days. Default
% is off
...
% Variance-explained threshold above which to do search.
if ~isfield(params.analysis,'hrfmins') || ~isfield(params.analysis.hrfmins.thresh,'ve')
    ...
    switch lower(params.analysis.grid.type)
        case 'triangular'           % find triangular grid positions
            if ~isfield(params.analysis,'X') || ~isfield(params.analysis,'Y')
                ...
                tmpmin=min([min(params.analysis.X) min(params.analysis.Y) 0]);
                tmpmax=max([max(params.analysis.X) max(params.analysis.Y) maxXY]);
                [tx,ty]=triangleGrid([tmpmin tmpmax],step);
            end
    ...
    switch lower(vararg{n}),
        ...
        case{'sigmaratiofixedvalue', 'sigma2fixedvalue','srf'}    
            params.analysis.sigmaRatioFixedValue = data(:);  % 3 values: 1st = sigma-sigma2 ratio 
                                                             ...
                                                             % 3rd = beta
        case {'hrffitthreshve'}
            params.analysis.hrfmins.thresh.ve = data;

        case {'hrffitthreshecc'}
            params.analysis.hrfmins.thresh.ecc = [0 data];    
            
        case {'refine','search fit refine parameter'}
            params.analysis.fmins.refine = data;
            
        case {'stimx'}
            params.analysis.X = data;
        
        case {'stimy'}
            params.analysis.Y = data;
            params.analysis.numberStimulusGridPoints=length(params.analysis.X)*length(params.analysis.Y);
            
        case {'stimfreq'}
            params.analysis.Freq=data;
            
        case {'stim timing per frame'}
            params.analysis.stimTimingPerFrame=logical(data);
            
        case {'separate run betas', 'separate betas'}
            params.seperateRunBetas=logical(data);
            
        case{'stimulus exponents', 'stimexps'}
            params.analysis.stimexps=data;
