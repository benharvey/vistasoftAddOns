...
% if size(M.params.analysis.X,1)*size(M.params.analysis.Y,1)~=M.params.analysis.numberStimulusGridPoints
%     if max(M.params.analysis.X)==2
%         M.params.analysis.X=[0.05:0.05:1 2];
%         M.params.analysis.Y=[0.05:0.05:1 2.1];
%     else
%         M.params.analysis.X=log([0.05:0.05:1 2])+3;
%         M.params.analysis.Y=log([0.05:0.05:1 2.1])+3;
%     end
%     [M.params.analysis.X, M.params.analysis.Y] = meshgrid(M.params.analysis.X,M.params.analysis.Y);
% end

if isfield(M.params, 'seperateRunBetas') && M.params.seperateRunBetas==1
    [pred RF rfParams variance_explained blanks] = rmPlotGUI_makePrediction(M, coords, [], M.params.seperateRunBetas);
else
...
if size(rfParams,1)<5 && rfParams(3)==rfParams(5)
	...
	% clear)
	txt = sprintf('sigma=%.4f%s', rfParams(3), degStr);
elseif size(rfParams, 1)==2
    txt = sprintf('sigma_1=%.4f, sigma_2=%.4f %s', rfParams(1,3), rfParams(2,3), degStr);
else
	% oval Gaussian: report both major and minor sigma
    sigmaTheta=round( rfParams(6) * (180/pi) );
	txt = sprintf('sigma_1=%.1f %s, sigma_2=%.1f %s, sigma_theta=%.2f %s CW from vert', rfParams(3), degStr, rfParams(5), degStr, sigmaTheta, degStr);
    
    if length(rfParams)>6
        txt = sprintf('sigma_1=%.1f %s, sigma_2=%.1f %s, sigma_theta=%.2f %s CW from vert, exp=%.2f', rfParams(3), degStr, rfParams(5), degStr, sigmaTheta, degStr, rfParams(7));
    end
