function model = rmSearchFit_oneGaussianPerVoxelHrf(model, data, params, wProcess, t)
...
for ii = 1:numel(wProcess),
    ...
    %Generate stim images convolved with voxel-specific HRF
    params.stim(1).images = params.stim(1).images_org.*(params.analysis.sampleRate.^2);
    params.stim(1).images = filter(params.analysis.Hrf{1}(:,ii), 1, params.stim(1).images');
    params.stim(1).images = params.stim(1).images(params.stim(1).prescanDuration+1:end,:);
    params.analysis.allstimimages=params.stim(1).images;
    ...
    if isnan(sum(X(:,1))); X(:,1)=zeros(size(X(:,1))); end
    b    = pinv(X)*vData;
