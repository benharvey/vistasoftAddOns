...
%This is because we now use lscov rather than pinv to calculate fits.
%Lscov also outputs the RSS, but the mean RSS per time point rather than
%the sum over all time points. This is what we will compare fits to.
...
%        nkeep   = b(1,:)<0 | b(2,:)<0 | b(3,:)<0 | b(4,:)<0 | b(5,:)<0;

%        %Even older
%        predCols=zeros(size(data,1), length(params.stim));
%         for stimRun=1:length(params.stim)
%             predCols(((stimRun-1)*framesPerScan+1):(framesPerScan*stimRun), stimRun)=prediction(((stimRun-1)*framesPerScan+1):(framesPerScan*stimRun),n);
%         end
%         X=[predCols trends];    
%         [b,ci,rss]    = lscov(X,data);
%         nkeep   = b(1:length(params.stim),:)<0;
%         nkeep = max(nkeep, [], 1);      
...
model.rss=single(model.rss.*(size(prediction,1)-size(trends,2)+1));  %Is this correct for separate run betas?
