function vw = rmRunLogDuration2dOval(vw,dataTypes, roi,wSearch,models, hrfParams, matFileName, maxCores, separateBetas, logIntensity)
...
gridStep=0.2/sampleRate; %Maybe smaller, also numberSigmas
blankValue=3.75;

if wSearch==9 || wSearch==10
    matFileName = sprintf('retModel-%s-LogMonotonic-DurationPeriod-DT0.5',datestr(now,'yyyymmdd-HHMMSS'));
elseif ischar(logIntensity) && strcmp(logIntensity,'free')
    matFileName = sprintf('retModel-%s-Log-2dOvalGaussian-DurationPeriod-DT0.5-maxValue-%s-expIntensity-free',datestr(now,'yyyymmdd-HHMMSS'), num2str(highestValue,2));
    logIntensity=[0.00001 0.2 0.4 0.6 0.8 1];
else
    matFileName = sprintf('retModel-%s-Log-2dOvalGaussian-DurationPeriod-DT0.5-maxValue-%s-expIntensity-%s',datestr(now,'yyyymmdd-HHMMSS'), num2str(highestValue,2), num2str(logIntensity,1));
end
    ...
    for n=1:numel(models)
        switch lower(models{n})
            case '1g'
                ...
                %matFileName = sprintf('retModel-%s-Log-2dOvalGaussian-DurationPeriod-DT0.5-maxValue-%s-logIntensity-%s',datestr(now,'yyyymmdd-HHMMSS'), num2str(highestValue,2), num2str(logIntensity,1)); 
