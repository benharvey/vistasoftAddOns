mapNames=["TTO", "TLO", "TPO", "TPC1", "TPC2", "TPC3", "TLS", "TF1", "TF2", "All"];
...
%                     end
                ...
                elseif whichSpace==3
                    nShapes=[1 2];
                elseif whichSpace==2 && whichProg==2 %Remove when models are done
                    nShapes=[1:3];
                    ...
                    for whichMap=1:length(mapNames)
                        if eval(char(strcat(['isfield(',thisData, ', ''',char(mapNames(whichMap)),''')'])))
                            for whichHemi=1:2%length(hemispheres)
                                targetData=char(strcat('dataAllSub.', mapNames(whichMap), '.',hemispheres(whichHemi),'.', modelSpace(whichSpace), '.', modelProgName(whichProg),'.', shapes(whichShape), '.', DTnames(whichDT)));
...
for whichDT=1:length(DTnames)
    for whichModel=1:length(modelNames)
        ...
        for whichMap=1:length(mapNames)
            if eval(char(strcat(['isfield(',thisData, ', ''',char(mapNames(whichMap)),''')'])))
                for whichHemi=1:2%length(hemispheres)
                    targetData=char(strcat('dataAllSub.', mapNames(whichMap), '.',hemispheres(whichHemi),'.Monotonic.', modelNames(whichModel), '.', DTnames(whichDT)));
                    %                             if ~exist('dataAllSub', 'var')
                    %                                 eval([targetData, '=[];'])
                    %                             end
