...
if numel(gridSigmas_unique)>1
   ...
   range.upper(3,:)=range.start(3,:);
   range.lower(3,:)=range.start(3,:);
end

% set boundary for second gaussian or minor axis (can't fit both yet)
if isfield(model, 's_minor') && sum(model.s~=model.s_major)>0
...
if isfield(model, 'exponent')
    ...
    range.start(fieldnum,:) = model.exponent;
    gridExps_unique=unique(params.analysis.exponent);
