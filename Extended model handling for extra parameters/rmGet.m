...
try
    switch lower(param),
        ...
        case {'fwhm', 'sigma2fwhm','fwhmax', 'sigma2fwhmax'}
            [val, tmp1, tmp2, tmp3, tmp4, tmp5] = rmGetDoGFWHM(model);
        case {'fwhmin', 'sigma2fwhmin'}
            [tmp1, val, tmp2, tmp3, tmp4, tmp5] = rmGetDoGFWHM(model);            
        ...
        case {'latx','laty','laterality x', 'laterality y'}
            ...
            % fit
        case {'b1','beta1'}
            val = model.beta(:,:,1); % assuming that model.beta(:,:,1) contains the beta values for the positive curve
            
        case{'b2','beta2'}
            val = model.beta(:,:,2); % same assumption as above. Assumption comes from the way in which rmGetDoGFWHM reconstructs the DoG
        ...
        case {'mean_polar_angle'}
            val       = cart2pol(model.meanX0, model.meanY0);
            val	      = mod(val, 2*pi);
        case {'mean_ecc'}
            [tmp val] = cart2pol(model.meanX0, model.meanY0);
        case {'rminusl'}
            val=model.RminusL;
