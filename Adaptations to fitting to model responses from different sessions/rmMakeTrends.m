...
if length(params.stim)>1 && isfield(params, 'seperateRunBetas') && params.seperateRunBetas
    start2 = [cumsum(ones(size(params.stim)))];
    for n = 1:numel(params.stim),
        % stimulus length
        tc = linspace(0,2*pi,tf(n))';
        % trends for one scan
        t(start1(n)+1:start1(n+1),[start2(n) start2(n)+numel(params.stim)]) = cos(tc*(0:0.5:params.stim(n).nDCT));  
        % keep track of dc components
        dcid(n) = start2(n);%+length(params.stim);
    end
else
    for n = 1:numel(params.stim),
        % stimulus length
