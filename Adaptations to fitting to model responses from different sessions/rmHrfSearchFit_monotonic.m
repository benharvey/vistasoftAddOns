function [view,params] = rmHrfSearchFit_monotonic(view,params)
...
switch desc,
    case {'1d prf fit (x,sigma, positive only)'}
        ...
        n = numel(tmp.x);
        s = [[1:ceil(n./100):n-2] n+1]; %#ok<NBRAK>
        tmp.prediction = zeros(size(allstimimages,1),n);
        fprintf(1,'[%s]:Making predictions without HRF for each voxel (%d):',mfilename,n);
        drawnow;tic;
        predtmp = allstimimages*params.analysis.X;
        for n=1:numel(s)-1,
            % make rfs
...
%                 tmp.y(s(n):s(n+1)-1));
            ...
            pred=zeros(size(tmp.prediction(:,s(n):s(n+1)-1)));
            tmptmp=tmp.x(s(n):s(n+1)-1);
            if any(tmptmp==1)
                pred(:,tmptmp==1)=repmat(predtmp, [1, sum(tmptmp==1)]);
            end
            if any(tmptmp==2)
                pred(:,tmptmp==2)=repmat(-predtmp, [1, sum(tmptmp==2)]);
            end
            
            
            % store
            tmp.prediction(:,s(n):s(n+1)-1) = pred;
            fprintf(1,'.');drawnow;
        end;
        clear n s rf pred;
