...
for slice=loopSlices,
    ...
    if isempty(ROIcoords),
        ...
        if strcmpi('inplane', viewGet(view, 'viewType'))
            ...
            if size(s{n}.b,2)==size(wProcess,1)
                sparseBetas=zeros([size(s{n}.b,1), size(s{n}.x0,2)]);
                sparseBetas(:,wProcess)=s{n}.b; %UNCOMMENT
                s{n}.b=sparseBetas;
            end
        end
        if vethresh>0
    ...
    for n=1:numel(model)
        ...
        switch desc,
            ...
            case {'double 2d prf fit (x,y,sigma,sigma2, center=positive)','d',...
                  ...
                  'compressive monotonic x plus compressive monotonic y(xcomp,ycomp, positive only)',...
                  'difference 1d prf fit (x,sigma, sigma2, center=positive)'}
              if params.analysis.fmins.options.MaxIter>0
    ...
    for n=1:numel(model)
        ...
        switch desc,
            case {'2d prf fit (x,y,sigma, positive only)','2', '1d prf fit with compressive monotonic y(x,ycomp,sigma, positive only)', 'compressive monotonic x plus compressive monotonic y(xcomp,ycomp, positive only)'}
            ...
            case{'difference 1d prf fit (x,sigma, sigma2, center=positive)'}
                % different submodel options
                s{n}=rmSearchFit_1DtwoGaussiansDoG(s{n},data,params,wProcess,t);
