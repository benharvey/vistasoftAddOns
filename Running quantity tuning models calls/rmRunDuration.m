function vw = rmRunDuration(vw,dataTypes, roi,wSearch,models, hrfParams, matFileName, maxCores, separateBetas)
    ...
    for n=1:numel(models)
        switch lower(models{n})
            case '1g'
                ...
                matFileName = sprintf('retModel-%s-Lin-1DGaussian-DT0.5-maxValue-%s',datestr(now,'yyyymmdd-HHMMSS'), num2str(highestValue,2)); 
            ...
            case 'dog'
                ...
                matFileName = sprintf('retModel-%s-Lin-1DDoGaussian-DT0.5-maxValue-%s',datestr(now,'yyyymmdd-HHMMSS'), num2str(highestValue,2));
